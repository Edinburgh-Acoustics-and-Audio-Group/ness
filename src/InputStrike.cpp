...
InputStrike::InputStrike(Component *comp, double x, double y, double z, double startTime,
    ...
    : InputSample(comp, comp->getAlpha(), x, y, z, interpolated, negated)
{
	...
	data[i] = 0.5 * amplitude * (1.0 - cos(M_PI * 2.0 * ((double)i) / ((double)this->duration)));
    }

    logMessage(1, "Strike on %s, start=%d, length=%d, position=%d", comp->getName().c_str(),
	       this->startTime, this->duration, index);
}

InputStrike::~InputStrike()
{
}
